...
public class LibroController {
        ...
        return libroService.guardarLibro(libroNuevo, dtoEntrada.getAutorId())
                ...
                .orElseGet(() -> {
                    ...
                    java.util.Map<String, String> errorDetails = java.util.Map.of(
                            "autorId", "Autor con ID " + dtoEntrada.getAutorId() + " no encontrado.");
    ...
    public ResponseEntity<JSendResponse> actualizarLibro(@PathVariable Long id,
        ...
        return libroService.actualizarLibro(id, libroDetalles, dtoEntrada.getAutorId())
                ...
                .orElseGet(() -> {
                    ...
                    errorDetails = java.util.Map.of("recurso", "Libro con ID " + id + " o Autor con ID "
                            + dtoEntrada.getAutorId() + " no encontrados para actualizar.");
    ...
    public ResponseEntity<JSendResponse> eliminarLibro(@PathVariable Long id) {
        ...
        /*
         * asi estaban antes los fallos PERO en las ppt dle prof dice que retorna un
         * objeto (aqui entran validaciones pero yo no puse xd)
         * return new ResponseEntity<>(
         * new JSendResponse("fail", null, "Libro con ID " + id +
    ...
    // @GetMapping("/search") si fuera una sola consulta personalizada (que ya se sabe el atributo a buscar),pero como no es, hay que especificar la ruta para evitar ambiguedad
    ...
    public ResponseEntity<JSendResponse> buscarLibroPorGenero(@RequestParam String genero) {        
        ...
        if (resultados.isEmpty()) {
            return ResponseEntity.ok(
                    new JSendResponse("success", List.of(), "No se encontraron libros que coincidan con el género."));
        }
    ...
    //para manejar OR el mapeo se hace excluyendo los atributos que no quieren que se consulten, en este caso, solo genero

    @GetMapping(value = "/search", params = "!genero")
    public ResponseEntity<JSendResponse> buscarLibroPorTituloOIsbn(
        //false porque es OR
            @RequestParam(required = false) String titulo,
            @RequestParam(required = false) String isbn) {
        // Validación que no haya nulls porque requestparam es false
        if (titulo == null && isbn == null) {
            return ResponseEntity.badRequest().body(
                    new JSendResponse("fail", null, "Se requiere al menos un parámetro"));
        }
        // al menos hay un parámetro, entonces:
        List<Libro> resultados = libroService.buscarPorTituloOIsbn(titulo, isbn);

        // 0 coincidencias
        if (resultados.isEmpty()) {
            return ResponseEntity.ok(
                    new JSendResponse("success", List.of(), "No se encontraron libros que coincidan."));
        }

        //si hay coincidencias, pero mapeamos a LibroDTO para mostrar info estratégica uy xd 
        List<LibroDTO> librosDTO = resultados.stream()
            .map(LibroDTO::new)
            .collect(Collectors.toList());
